...
float odds(int pir, float temperatur, float co2, float ljud, float ljus)
{
  ...
  const unsigned int EE_safecode = 404;   //mindre värde än 65536, används för att se så att det är rätt version/data på EEPROM
  ...
  if (counter_in >= 1 && counter_in != counter)                   //beräknar oddsen för närvaro enligt valda sensorer
  {
    /*float asdf = (float(counter_pir_there)/float(counter_in));
    float derpsasd = (float(counter_pir_not) / (float(counter) - float(counter_in)));
    Serial.print("\rDetektionF = (");
    Serial.print(counter_pir_there);
    Serial.print("/");
    Serial.print(counter_in);
    Serial.print(")/((");
    Serial.print(asdf);
    Serial.print(")+(");
    Serial.print(derpsasd);
    Serial.print(")) = ");*/
    pir_odds_there = (float(counter_pir_there) / float(counter_in)) / ((float(counter_pir_there) / float(counter_in)) + (float(counter_pir_not) / (float(counter) - float(counter_in))));
    /*Serial.print(pir_odds_there);
    Serial.print("\t antal:");
    Serial.println(counter);*/
    pir_odds_not = (1-(float(counter_pir_there) / float(counter_in)))/((1-(float(counter_pir_there) / float(counter_in))) + (1-(float(counter_pir_not) / (float(counter)-float(counter_in)))));

    ljud_odds_there = (float(counter_ljud_there)/ float(counter_in)) / ((float(counter_ljud_there) / float(counter_in)) + (float(counter_ljud_not) / float(counter-counter_in)));
    ljud_odds_not = (1-(float(counter_ljud_there) / float(counter_in)))/((1-(float(counter_ljud_there) / float(counter_in))) + (1-(float(counter_ljud_not) / (float(counter)-float(counter_in)))));
    
    co2_odds_up = (float(counter_co2_up) / float(counter_in)) / ((float(counter_co2_up) / float(counter_in)) + (float(counter_co2_upnot) / (float(counter)-float(counter_in))));
    co2_odds_down = (float(counter_co2_down) / float(counter_in)) / ((float(counter_co2_down) / float(counter_in)) + (float(counter_co2_downnot) / (float(counter)-float(counter_in))));
    
    temp_odds_up = (float(counter_temp_up) / float(counter_in)) / ((float(counter_temp_up) / float(counter_in)) + (float(counter_temp_upnot) / (float(counter)-float(counter_in))));
    temp_odds_down = (float(counter_temp_down) / float(counter_in)) / ((float(counter_temp_down) / float(counter_in)) + (float(counter_temp_downnot) / (float(counter)-float(counter_in))));
    temp_odds_still = (float(counter_temp_still) / float(counter_in)) / ((float(counter_temp_still) / float(counter_in)) + (float(counter_temp_stillnot) / (float(counter)-float(counter_in))));
  }
  ...
  if (counter < use_temp) {             //Har det gått tillräckligt med tid innan co2/temp startat ?
  ...
  else {                                //Beräka odds för co2, temperatur
    ...
    if (tempval < 0) {
      ...
      temp_weight = 0.25 + 1.5 * abs((temp_odds_down - 0.5));
      if (inut_tot >= 1) {
        counter_temp_down++;
      }
      else {
      ...
      temp_weight = 0.25 + 1.5 * abs((temp_odds_up - 0.5));
      if (inut_tot >= 1) {
        counter_temp_up++;
      }
      else {
      ...
      temp_weight = 0.25 + 1.5 * abs((temp_odds_still - 0.5));
      if (inut_tot >= 1) {
        counter_temp_still++;
      }
      else {
    ...
    if (co2val < 0) {
      ...
      co2_weight = 0.25 + 1.5 * abs((co2_odds_down - 0.5));
      if (inut_tot >= 1) {
        counter_co2_down++;
      }
      else {
      ...
      co2_weight = 0.25 + 1.5 * abs((co2_odds_up - 0.5));
      if (inut_tot >= 1) {
        counter_co2_up++;
      }
      else {
  ...
  if (ljud > 730) { //(730/630)*microphone_Xe)
    ...
    ljud_weight = 0.25 + 1.5 * abs((ljud_odds_there - 0.5));
    if (inut_tot >= 1) {
    ...
    ljud_weight = 0.25 + 1.5 * abs((ljud_odds_not - 0.5));
  }

  if (pir >= 1) {
    pir_weight = 0.25 + 1.5 * abs((pir_odds_not - 0.5));
